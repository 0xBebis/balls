...
## Enhanced AI with dash ability, varied behavior, and smarter targeting.
...
## AI tuning
const BASE_STEERING_FORCE: float = 900.0
const JITTER_STRENGTH: float = 60.0
const TARGET_UPDATE_INTERVAL: float = 0.15

## Dash ability
const DASH_COOLDOWN: float = 3.0
const DASH_STRENGTH: float = 600.0
const DASH_TRIGGER_DISTANCE: float = 150.0

## Behavior weights
var aggression: float = 0.5  # 0 = defensive, 1 = aggressive
var reaction_speed: float = 0.5  # How quickly AI responds
var dash_tendency: float = 0.5  # How often AI uses dash
...
func initialize(ball: Node2D, combat: CombatSystem, seed_value: int = 0) -> void:
	...
	aggression = rng.randf_range(0.3, 1.0)
	reaction_speed = rng.randf_range(0.4, 1.0)
	dash_tendency = rng.randf_range(0.2, 0.8)

	# Randomize initial dash cooldown
	dash_cooldown_timer = rng.randf() * DASH_COOLDOWN * 0.5
...
func _physics_process(delta: float) -> void:
	...
	if target_update_timer <= 0:
		target_update_timer = TARGET_UPDATE_INTERVAL * (2.0 - reaction_speed)
		_update_target()
...
func _calculate_target_score(target: Node2D) -> float:
	...
	var distance_score: float = 1000.0 / max(distance, 50.0)

	# Prefer low HP targets (more aggressive = more preference)
	var hp_score: float = (1.0 - hp_ratio) * 50.0 * aggression

	# Add some randomness
	var random_score: float = rng.randf() * 20.0
...
func _execute_behavior(delta: float) -> void:
	...
	is_kiting = distance < ideal_distance * 0.7 and aggression < 0.7

	# Check if we need to evade
	if evade_timer > 0:
		_apply_steering(evade_direction, delta)
		return

	# Check for incoming threats
	if _should_evade():
		evade_direction = _calculate_evade_direction()
		evade_timer = 0.3 + rng.randf() * 0.2
		_apply_steering(evade_direction, delta)
		return
	...
	if is_kiting:
		...
		kite_direction = (kite_direction + strafe * 0.5).normalized()
		_apply_steering(kite_direction, delta)
	else:
		# Aggressive pursuit
		_apply_steering(direction, delta)

func _apply_steering(direction: Vector2, _delta: float) -> void:
	# Add jitter using time-based noise
	var time: float = Time.get_ticks_msec() / 1000.0
	var jitter_x: float = sin((time + noise_offset) * 3.5) * JITTER_STRENGTH
	var jitter_y: float = cos((time + noise_offset) * 2.9) * JITTER_STRENGTH
	...
	# Scale steering force by aggression
	var force_multiplier: float = 0.8 + aggression * 0.4
...
func _wander(delta: float) -> void:
	...
	var time: float = Time.get_ticks_msec() / 1000.0
	var wander_x: float = sin((time + noise_offset) * 0.7)
	var wander_y: float = cos((time + noise_offset) * 0.5)
	var wander_direction: Vector2 = Vector2(wander_x, wander_y).normalized()

	_apply_steering(wander_direction, delta)
...
func _get_ideal_distance() -> float:
	...
	if owner_ball.weapon:
		...
		match weapon_type:
			"bow":
				return 250.0  # Stay far for ranged
			"spear":
				return 120.0  # Medium range
			"dagger":
				return 50.0   # Get close
			_:
				return 80.0   # Default melee range
	...
	for ball in balls:
		...
		if distance < 100.0:
			...
			if closing_speed > 300.0:
				return rng.randf() < reaction_speed * 0.5
...
func _calculate_evade_direction() -> Vector2:
	...
	for ball in balls:
		...
		if distance < 150.0 and distance > 0:
...
func _try_dash() -> void:
	...
	# Offensive dash - close distance quickly
	if distance > DASH_TRIGGER_DISTANCE and distance < 300.0:
		if rng.randf() < dash_tendency * aggression * 0.3:
			should_dash = true
	...
	if hp_ratio < 0.3 and rng.randf() < dash_tendency * 0.5:
		should_dash = true
		dash_direction = -to_target.normalized()

	# Engagement dash - dash to the side for flanking
	if distance < 100.0 and rng.randf() < dash_tendency * 0.2:
		should_dash = true
...
func _perform_dash(direction: Vector2) -> void:
	dash_cooldown_timer = DASH_COOLDOWN * (1.5 - dash_tendency * 0.5)
